...
const linesBetween = (top, bottom, scope, matchers, guides, callback) => {
  for (const guide of guides) {
    ...
    if ('level' in guide) { contraints.push(guide.level === scope.level); }
...
module.exports = (scope, guides, context, sourceCode) => {
  const comments = sameStartAsParent(scope.current) ? [] : sourceCode.getCommentsBefore(scope.current);
  const nodes = [...comments, scope.current];
  if (scope.prev) { nodes.unshift(scope.prev); }
