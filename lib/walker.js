...
const beginBracker = utils.beginBracker;
const endBracker = utils.endBracker;
...
const walk = function (node, sourceCode, options) {
  ...
  nodes.forEach(n => {
    ...
    if (children && children.length) {
      enter(n);
      walk(beginBracker(n, sourceCode), sourceCode, options);
      ...
      walk(endBracker(n, sourceCode), sourceCode, options);
      leave(n);
    }
  });
};
